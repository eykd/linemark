...
@pytest.mark.asyncio
async def test_list_types_shows_all_document_types() -> None:
...
"""
    ...
    # Act
    types = await use_case.list_types(sqid='SQID1', directory=directory)
...
@pytest.mark.asyncio
async def test_list_types_returns_empty_for_nonexistent_sqid() -> None:
    ...
    # Act
    types = await use_case.list_types(sqid='NONEXISTENT', directory=directory)
...
@pytest.mark.asyncio
async def test_add_type_creates_new_file() -> None:
...
"""
    ...
    # Act
    await use_case.add_type(sqid='SQID1', doc_type='characters', directory=directory)
...
@pytest.mark.asyncio
async def test_add_type_preserves_existing_files() -> None:
...
@pytest.mark.asyncio
async def test_add_type_raises_error_if_type_already_exists() -> None:
...
"""
    ...
    with pytest.raises(ValueError, match='Type draft already exists'):
        await use_case.add_type(sqid='SQID1', doc_type='draft', directory=directory)


@pytest.mark.asyncio
async def test_add_type_raises_error_if_sqid_not_found() -> None:
    ...
    with pytest.raises(ValueError, match='Node with SQID NONEXISTENT not found'):
        await use_case.add_type(sqid='NONEXISTENT', doc_type='characters', directory=directory)


@pytest.mark.asyncio
async def test_remove_type_deletes_file() -> None:
...
"""
    ...
    # Act
    await use_case.remove_type(sqid='SQID1', doc_type='characters', directory=directory)
...
@pytest.mark.asyncio
async def test_remove_type_raises_error_for_required_types() -> None:
...
"""
    ...
    with pytest.raises(ValueError, match='Cannot remove required type'):
        await use_case.remove_type(sqid='SQID1', doc_type='draft', directory=directory)

    # Act & Assert - cannot remove notes
    with pytest.raises(ValueError, match='Cannot remove required type'):
        await use_case.remove_type(sqid='SQID1', doc_type='notes', directory=directory)


@pytest.mark.asyncio
async def test_remove_type_raises_error_if_type_not_found() -> None:
...
@pytest.mark.asyncio
async def test_remove_type_raises_error_if_sqid_not_found() -> None:
    ...
    # Act & Assert
    with pytest.raises(ValueError, match='Node with SQID NONEXISTENT not found'):
        await use_case.remove_type(sqid='NONEXISTENT', doc_type='characters', directory=directory)
